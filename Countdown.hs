...
solution e ns i       = elem (values e) (subbags ns) && eval e == [i]
...
infExpr = App Sub (App Sub (Val 1) (Val 5)) infExpr



gen_exprs ns = exprs ns 

m_Solutions            :: [Int] -> Int -> [Expr]
m_Solutions ns i        = [e | ns' <- subbags ns, e <- gen_exprs ns', eval e == [i]]
...
m_Solutions'           :: [Int] -> Int -> [Expr]
m_Solutions' ns i       = [e | ns' <- subbags ns, (e,j) <- results ns', j == i]
...
instance Show Expr where
   ...
   show (App o l r)   = bracket l ++ show o ++ bracket r
                        where
                           bracket (Val i) = show i
                           bracket e       = "(" ++ show e ++ ")"

display              :: [Expr] -> IO ()
display []            = putStr "\nThere are no m_Solutions.\i\i"
display (e:es)        = do putStr "\nOne possible solution is "
	                   ...
	                   putStr ".\i\nPress return to continue searching..."
                           ...
                           putStr "\i"
                           if null es then
                               putStr "There are no more m_Solutions.\i\i"
                            else
                               do sequence [print e | e <- es]
                                  putStr "\nThere were "
                                  putStr (show (length (e:es)))
                                  putStr " m_Solutions in total.\i\i"
...
prop_solutions (ns, j) = m_Solutions ns j == m_Solutions' ns j


main = putStrLn $ show $ m_Solutions [1,3,7,10,25,50] 765
